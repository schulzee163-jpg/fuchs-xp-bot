...
/* 🐺 RUDEL-KAMPF ANIMATION */
#linksRudel,
#rechtsRudel {
  transition: transform 0.7s ease;
}

.box.angriff #linksRudel {
  transform: translateX(70px) scale(1.25);
}

.box.angriff #rechtsRudel {
  transform: translateX(-70px) scale(1.25);
}
...
async function chatVerarbeiten(message) {
  ...
  const pvpMatch =
    text.match(/^!pvp\s+@?([a-zA-Z0-9_]+)$/i);

  if (pvpMatch) {
    const antwort =
      await pvpStart(username, pvpMatch[1]);
  ...
  const pokemonMatch =
    text.match(/^!pokemon(?:\s+(.+))?$/i);

  if (pokemonMatch) {
    await streamelementsSenden(
      await pokemonWahl(username, pokemonMatch[1])
    );
  ...
  const pokemonKampfMatch =
    text.match(/^!pokekampf\s+@?([a-zA-Z0-9_]+)$/i);
  ...
  const rudelMatch =
    text.match(/^!rudelwahl\s+(.+)$/i);

  if (rudelMatch) {
    await streamelementsSenden(
      await rudelwahl(username, rudelMatch[1])
    );

    return;
  }

  if (/^!quest$/i.test(text.trim())) {
    await persoenlicheQuestAnzeigen(username);
    return;
  }

  if (/^!allebefehle$/i.test(text.trim())) {
